...
import { absoluteApiUrl, interactionKeys, interactionsApi, normalizeAnnotationPosition, useShareComments, useSharedPicture } from "@/features/interactions";
...
export function ShareView({ publicId }: { publicId: string }) {
  ...
  useEffect(() => {
    ...
    queueMicrotask(() => {
      setSecret(value);
      if (!value) { setAccessError("分享链接缺少访问密钥"); setChecking(false); return; }
      verify(value);
    });
  }, [publicId, verify]);
  ...
  const annotations = (comments.data?.items ?? []).filter((comment) => comment.kind === "annotation" && !comment.deleted && comment.pictureVersionId === item.currentVersionId && comment.x != null && comment.y != null);
