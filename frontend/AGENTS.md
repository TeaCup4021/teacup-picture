...
本文件适用于 `frontend/`。产品范围以 `../docs/product-prd.md` 为准，当前后端能力以 `../backend/`、Flyway 和 `../docs/openapi/*.yaml` 为准，视觉与交互以 `../docs/ui-design/README.md` 为准。

## 当前阶段

当前已经实现 M1～M3 和 M6；M5-R 已有多人协作基础闭环：

- `/`：匿名公开图库。
- `/pictures/[pictureId]`：图片详情；公开图片允许匿名访问，私有图片依赖会话和服务端权限；已接入下载、公开撤回、分享管理、评论、回复、结构化提及和版本绑定位置批注。
- `/login`、`/register`：账号与 Cookie 会话。
- `/spaces/personal`：个人图片和公开审核状态。
- `/upload`：本地文件或图片 URL 导入个人空间。
- `/admin/reviews`：平台管理员处理公开申请。
- `/ai/create`、`/ai/tasks`：AI 创作与任务。
- `/editor/[pictureId]`：Fabric.js MIT 单人结构化编辑器、`EditorState v3`（兼容 v2）、revision 草稿自动保存、替换/另存、三动作退出确认和不可变版本历史。
- `/shares/[publicId]`：片段密钥校验、可选访问密码、Session 授权图片查看、登录下载和讨论续办。

M4 邀请流程、M7 分析和 M5 分布式生产加固仍是后续能力。对应规划页面不代表路由、接口、依赖或数据模型已就绪。
...
- Zustand、ECharts、WebSocket 和协作库只能在对应里程碑需要且架构决策完成后使用；M3-R 已确认 Fabric.js MIT 为单人编辑交互内核，但后端契约不得保存 Fabric 私有 JSON。
...
- 登录默认进入角色首页；M6 分享页支持经过本站相对路径校验的 `returnTo`，其他页面尚未统一接入登录续办。
- 图片详情已提供 M6 下载、分享、评论和批注；缩放仍未实现。分享完整链接只在生成/重新生成时显示一次。
- 主题切换和深色全站主题尚未实现；当前基线为浅色，编辑器页面已独立使用深色工作区。
- 编辑器已覆盖选择、裁切、画笔、马克笔、精细擦除、文字、旋转、缩放、内部对象和 PRD 全量调节；界面不展示独立图层列表，后续变更继续维持预览/导出一致性。
- 编辑器保存由前端导出扁平 PNG 后经后端写入 MinIO，可替换当前图片或另存同空间私有副本；前端不直接上传或持有对象存储凭据。
