...
- `/editor/[pictureId]`：Fabric.js MIT 单人结构化编辑器、`EditorState v2`、revision 草稿自动保存、三动作退出确认和不可变版本历史，已完成 M3-R 全量验收。
...
- 编辑器已覆盖选择、裁切、画笔、马克笔、擦除、文字、旋转、缩放、图层和 PRD 全量调节；后续变更继续维持预览/导出一致性和现有关闭标准。
- 编辑器版本预览由前端导出 PNG 后经后端写入 MinIO；前端不直接上传或持有对象存储凭据。
