...
| 完成 | M1 `/api/v1` 契约 | 认证、个人空间、上传、图片管理、发布审核和公开只读接口已由 v1 适配层实现；可恢复分片上传接口已增加，单张上限仍为 20 MB |
| 完成 | M1 个人空间 | 新注册事务、幂等创建及本地/URL 上传的默认空间定位已完成；存量补偿另行处理 |
| 完成 | M1 公开审核闭环 | 独立申请记录、管理员审核/撤回、匿名公开列表与详情已实现 |
| 完成 | 公开读多级缓存 | Caffeine 本地缓存（键含共享版本号）+ Redis 共享缓存 + 共享版本号轮询同步；详情类精确删除、列表类版本号切换；失效仅两条幂等缓存命令，ETag/304 与公开版本 URL 已实现；私有实体缓存已关闭 |
...
- 2026-09-25：空间额度改为「已用 + 在途预留 + 本次 ≤ 上限」的条件更新，`V26__add_space_reserved_size.sql` 为 `space` 增加 `reservedSize`。上传会话在建会话时即占住额度，完成时结转、取消与过期清理时释放。所有额度收支统一走 `SpaceMapper` 的额度入口（占用 / 预留 / 结转 / 释放），修复了两类问题：并发下多个会话各自通过「先读后判」的校验、全部完成后超额占用空间；旧接口更新图片时只加不减，导致旧图大小永久留在已用额度里、件数被重复累加。
- 2026-09-07：新增后端管理的可恢复分片上传会话（5 MiB 分片、24 小时过期、重复分片幂等、取消和定时清理），完成后仍复用 `PictureStorage` 的图片校验与缩略图流程；前端对大于 8 MiB 的本地图片自动切换分片上传，并通过浏览器会话缓存已上传分片。
...
- 公开图片列表/详情走两层缓存：本地缓存键含共享版本号（版本号一变即整体逻辑失效，不需要遍历清理），共享缓存中详情不含版本号、列表含版本号；负值缓存挡住不存在的编号；共享缓存不可用时直接回源、不返回陈旧值。审核、撤回、编辑替换、版本恢复与兼容旧接口写入统一在事务提交后执行两条幂等命令（删除该图的详情键 + 版本号递增）。旧 `/api/**` 私有图片/空间实体缓存和 `SCAN` 失效逻辑已移除。
...
- 评论属于 M6，不在 M1 契约范围；公开缓存实现已落地，生产部署仍须为 Redis/RabbitMQ 配置独立容量、告警和受保护的指标采集。
...
- 上传已限制 20 MB、最长边 16,384 像素和总像素 4,000 万，解码前读取尺寸并只按缩略图需要子采样；后续仍需补更多畸形图片专项样本。
- WebSocket Origin 白名单、Cookie 鉴权、连接限额、消息大小和频率限制。
- 图片已统一写入 Docker Compose 中的私有 MinIO bucket，并通过后端鉴权资源接口访问；运行时不再回退本地文件或 COS。URL 预览使用 10 分钟、100 MB 上限的进程内临时文件缓存并以用户绑定的一次性令牌复用原图；多实例部署前需改为共享临时对象或配置会话粘滞。后续仍需补公开派生图、CDN、备份恢复和分享访问控制，但不得引入并行图片存储实现。
